...
const InvestmentsView: React.FC<InvestmentsViewProps> = ({ state, onUpdateGoal }) => {
  ...
  // Dummy Portfolio Data - Enhanced with new fields
  ...
  const [portfolio, setPortfolio] = useState<Investment[]>([
    { 
      ...
      shares: 15, 
      type: 'stock',
      description: 'Tech giant exposure for long-term growth. Part of the core equity strategy.',
      totalInvested: 2400.00,
      linkedGoalId: '3', // Matches 'New Car' in App.tsx
      contributions: [
        { date: '2023-01-15', amount: 1200.00 },
        { date: '2023-06-20', amount: 1200.00 }
      ]
    },
    { 
      symbol: 'BTC', 
      ...
      shares: 0.05, 
      type: 'crypto',
      description: 'High risk/reward allocation. Digital store of value experiment.',
      totalInvested: 2500.00,
      contributions: [
        { date: '2023-03-10', amount: 2500.00 }
      ]
    },
    { 
      symbol: 'VTI', 
      ...
      shares: 40, 
      type: 'etf',
      description: 'Broad market coverage for stability and consistent returns. Foundational asset.',
      totalInvested: 8500.00,
      linkedGoalId: '2', // Matches 'Emergency Fund' in App.tsx
      contributions: [
        { date: '2022-11-01', amount: 5000.00 },
        { date: '2023-05-15', amount: 3500.00 }
      ]
    },
    { 
      symbol: 'NVDA', 
      ...
      shares: 5, 
      type: 'stock',
      description: 'AI sector bet. High volatility expected but strong upside potential.',
      totalInvested: 3800.00,
      linkedGoalId: '1', // Trip to Japan
      contributions: [
        { date: '2023-08-01', amount: 3800.00 }
      ]
    },
  ]);
  ...
  const previousValue = portfolio.reduce((acc, curr) => acc + ((curr.price - curr.change) * curr.shares), 0);
  const dailyGain = totalValue - previousValue;
  const dailyGainPercent = (dailyGain / previousValue) * 100;
  const totalReturnPercent = 12.4; 
  ...
  const handleContribute = (e: React.FormEvent) => {
    ...
    // 1. Update Local Portfolio State
    const updatedPortfolio = portfolio.map(asset => {
      if (asset.symbol === selectedAsset.symbol) {
        const newShares = amount / asset.price; // Simplified buy at current price
        return {
          ...asset,
          shares: asset.shares + newShares,
          totalInvested: (asset.totalInvested || 0) + amount,
          contributions: [
            { date: new Date().toISOString().split('T')[0], amount: amount },
            ...(asset.contributions || [])
          ]
        };
      }
    ...
    // 2. Update Linked Goal Global State (if applicable)
    ...
    // 3. Reset and Close
    const updatedAsset = updatedPortfolio.find(a => a.symbol === selectedAsset.symbol) || null;
    setSelectedAsset(updatedAsset); // Keep overlay open with new data
    setContributionAmount('');
    alert(`Aporte de R$ ${amount} realizado com sucesso em ${selectedAsset.name}!`);
  };
