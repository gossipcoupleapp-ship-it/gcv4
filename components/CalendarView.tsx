...
const CalendarView: React.FC<CalendarViewProps> = ({ events, goals = [], onAddEvent }) => {
   ...
   const handleSync = async () => {
      ...
      try {
         ...
         alert(`Sync completed! ${data.count} events updated.`);
      } catch (err) {
      ...
      <div className="space-y-6 animate-fade-in pb-12 relative">
         ...
         <div className="flex items-center justify-between bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
            <div>
               ...
               <p
                  onClick={handleSync}
                  className="text-sm text-gray-500 flex items-center mt-1 cursor-pointer hover:text-green-600 transition-colors"
               >
                  <span className={`w-2 h-2 ${isSyncing ? 'bg-yellow-400 animate-pulse' : 'bg-green-500'} rounded-full mr-2`}></span>
                  {isSyncing ? 'Syncing...' : 'Synced with Google Calendar'}
                  {!isSyncing && <RefreshCw size={12} className="ml-2 opacity-50" />}
               </p>
            </div>
